...
export default {
    ...
    methods: {
        ...
        x22x() {
            ...
            //^ c  оформить  каждую  анимацию  как  отдельный  класс(с  методами  start(cfg)  stop()  и  событием  изнутри  анимации  onStop  ) и импортировать параметр cfg,
            // переданный в анимацию, и нужно анимировать(он может быть внешним реактивным объектом
            //), а не внутреннием переменные анимации
            //
            //
            // учесть может - ли быть много экземпляров одной анимации, например много одинаковых искр от летящего снаряда
            //
            // реализовать "пропуск кадров",
            // чтобы анимация могла рисоваться с нужой скоростью, хоть и с меньшей частотой кадров
        },
