...
~~
create sequence g_UsrAttr start with 1000 increment by 1
~~
...
~~
insert into TagType (id,text,code,deleted)
values (1008,'Свойства звукового файла','word-sound-info',null)
~~
insert into TagType (id,text,code,deleted)
...
~~
alter table UsrAttr add constraint pk_UsrAttr primary key (id)
~~
...
~~
create unique index i_UsrAttr_idx_usr_name on UsrAttr(usr,name)
~~
...
foreign key(dataType) references DataType(id)
~~
alter table UsrAttr add constraint fk_UsrAttr_usr
foreign key(usr) references Usr(id)
~~
...
~~
create index fki_UsrAttr_usr on UsrAttr(usr)
~~
