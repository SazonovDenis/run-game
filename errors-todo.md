...
если искать "dog" - то в переводах есть "{астр.}:" т.е. превода нет

# Сделать расцветку тэгов хотя бы по тупому

# Дочистить базовые словари

Миграция: если слово уже есть, то переводов не добавлять (и транскрипций тоже), только
пополнять транскипции или звуки (слово "cat" встречается три раза, со звкуом)

# Фильтр списка планов

Добавить 1-3 (не более) кнопок готовых фильтров (по типу "РУС" "КАЗ").

- сложные/простые
- ???

Более подробные (например, по темам или урокам) могут появлятся по нажатии "поиска по
слову" и исчезать вместе с ним.

# Для русских слов сделать учет ё->е

в факте word-spelling-distorted

# Верстка редактирования плана поехала - лишние скролы и прочее

В окне фото

# Список top-1000-2000-3000 англ слов

https://w2mem.com/words/en/1/

Вроде неплох - слов много, переводы короткие. Сделать обработку и объединение

~~~
data/web-grab/3000-w2mem.com/
~~~

# Еще 2 казахских словаря

1.

Большой, академический, но требует парсинга (отсканированный)

~~~
data/web-grab/kz.sozdik/bektaev.pdf 
~~~

2.

Поменьше, но зато сразу под парсинг

~~~
data/web-grab/kz.dict/3875_sozdik.doc 
~~~


# В перводах so неправильный вариант

so - "указывает на" - он в словаре италиком, можно отсечь
subscribe - "to" НЕ италиком, но в скобках
beyond - "указывает на"

Сделать 
 - не допускать появления таких слов в переводах
 - распространение таких префиксов на более нижние уровни (по номерам)


# Парсинг: убрать лишние собачки в конце строк и в середине


# Искали "wounds". Нашли wounds (клянусь), но не нашли wound (рана)